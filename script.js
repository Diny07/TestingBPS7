...
function initDashboardPage() {
  ...
  async function loadAllData() {
    try {
      const [pengaduanSnap, saranSnap, gratifikasiSnap] = await Promise.all([
      ...
      pengaduanSnap.forEach((doc) => {
        ...
        if (data.sumber === "Offline") {
          ALL_OFFLINE.push({ id: doc.id, ...data });
        } else {
          ALL_REPORTS.push({ id: doc.id, ...data });
        }
      });
  ...
  function renderTrendChart() {
    ...
    const semuaLaporan = [...ALL_REPORTS, ...ALL_OFFLINE, ...ALL_GRATIFIKASI];
      ...
      .map((m, i) => {
        ...
        return `
          <div class="bar-group" title="${m.label}: ${masukPerBulan[i]} masuk, ${selesaiPerBulan[i]} selesai">
            ...
            <div class="bar selesai" style="height:${hSelesai}px; background: var(--merah-terang);"></div>
          </div>`;
      })
      .join("");
  ...
  function renderCategoryStats() {
    ...
    const counts = [
      { label: "Pengaduan Warga", count: ALL_REPORTS.length, color: "#e74c3c" },
      { label: "Input Offline", count: ALL_OFFLINE.length, color: "#3498db" },
      { label: "Antigratifikasi", count: ALL_GRATIFIKASI.length, color: "#f39c12" },
      { label: "KoSan", count: ALL_SARAN.length, color: "#2ecc71" }
    ];
  ...
  function renderSaranTable() {
    ...
    if (!tbody) return;
    tbody.innerHTML = ALL_SARAN.map((s) => `
      <tr>
  ...
  function renderGratifikasiTable() {
    ...
    if (!tbody) return;
    tbody.innerHTML = ALL_GRATIFIKASI.map((g) => {
  ...
  function renderOfflineTable() {
    ...
    const filtered = ALL_OFFLINE.filter((o) => {
      const searchTerm = CURRENT_SEARCH.toLowerCase();
      if (!searchTerm) return true;
      const haystack = [
        o.petugas || o.namaPegawai || "",
        o.nama || "",
        o.alamat || "",
        o.telepon || "",
        o.materiPengaduan || o.materi || "",
        o.jawabanPetugas || o.tanggapan || "",
        formatTanggal(o.tanggalPengaduan || o.tanggal)
      ].join(" ").toLowerCase();
    ...
    tbody.innerHTML = filtered.map((o) => `
      <tr>
        <td>${escapeHTML(o.petugas || o.namaPegawai || "-")}</td>
...
function initOfflinePage() {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    if (!tanggalPengaduan || !nama || !alamat || !noHp || !namaPegawai || !materiPengaduan || !jawabanPetugas || !file) {
      if (errorAlert) {
        errorAlert.style.display = "block";
        errorAlert.textContent = "Mohon lengkapi semua kolom wajib, termasuk nama pegawai dan foto Dokumentasi.";
      }
      window.scrollTo({ top: 0, behavior: "smooth" });
      return;
    }

    if (namaPegawai.length < 2) {
      if (errorAlert) {
        errorAlert.style.display = "block";
        errorAlert.textContent = "Nama pegawai minimal 2 karakter agar dapat diketahui pencatat konsultasi.";
      }
      document.getElementById("namaPegawai")?.focus();
      return;
    }
...
function initIndexMonitorPage() {
  ...
  function renderMonitorStats(total, menunggu, selesai, totalSaran) {
    animateNumber(totalEl, total);
    animateNumber(menungguEl, menunggu);
    animateNumber(selesaiEl, selesai);
    if (saranEl) animateNumber(saranEl, totalSaran);
  }

  async function loadMonitorStats() {
    try {
      const [pengaduanSnap, saranSnap] = await Promise.all([
        getDocs(collection(db, "pengaduan")),
        getDocs(collection(db, "saran"))
      ]);
      ...
      pengaduanSnap.forEach((docSnap) => {
        ...
        total += 1;
        if (normalizeStatus(data.status) === "Menunggu") {
          menunggu += 1;
        } else {
          selesai += 1;
        }
      ...
      const totalSaran = saranSnap.size;
      const stats = { total, menunggu, selesai, saran: totalSaran };
      localStorage.setItem("pritkotas-monitor-fallback", JSON.stringify(stats));
      renderMonitorStats(total, menunggu, selesai, totalSaran);
    } catch (error) {
      console.error("Gagal memuat data monitoring:", error);

      const fallback = localStorage.getItem("pritkotas-monitor-fallback");
      if (fallback) {
        try {
          const parsed = JSON.parse(fallback);
          renderMonitorStats(Number(parsed.total || 0), Number(parsed.menunggu || 0), Number(parsed.selesai || 0), Number(parsed.saran || 0));
          return;
        } catch (parseError) {
          console.error("Gagal membaca cache monitoring:", parseError);
        }
      }

      if (error && error.code === "permission-denied") {
        showToast("Data monitoring belum bisa dimuat karena aturan akses Firestore belum diperbarui. Silakan aktifkan izin baca publik untuk koleksi pengaduan dan saran.", "error");
      }

      renderMonitorStats(0, 0, 0, 0);
    }
  }
